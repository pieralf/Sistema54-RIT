...
export default function NuovoClientePage() {
  ...
  // Stato per gestire il processo a step per nuovi clienti multisede con noleggio
  const [currentStep, setCurrentStep] = useState<1 | 2>(1); // Step 1: anagrafica + sedi, Step 2: macchine noleggio
  const [clienteIdStep1, setClienteIdStep1] = useState<number | null>(null); // ID cliente dopo step 1
  ...
  useEffect(() => {
    if (isEdit && id) {
      loadCliente();
      setCurrentStep(2); // In modifica siamo sempre allo step 2 (tutto disponibile)
      setClienteIdStep1(parseInt(id));
    } else if (id && !isEdit) {
      // Se c'è un ID ma non siamo in modifica, significa che siamo nello step 2 dopo la creazione
      setClienteIdStep1(parseInt(id));
      setCurrentStep(2);
      // Carica le sedi del cliente creato
      loadCliente();
    } else {
      // Reset step quando si crea un nuovo cliente
      setCurrentStep(1);
      setClienteIdStep1(null);
    }
  }, [id, isEdit]);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // Prepara assets per l'invio (solo se non è nuovo cliente con multisede)
      const assetsToSend = (!isEdit && hasMultisede) ? [] : (hasNoleggio ? assetsNoleggio.map((asset: any) => {
        const { tipo_configurazione_bn, tipo_configurazione_colore, ...assetClean } = asset;
      ...
      // STEP 1: Crea/Aggiorna cliente base con sedi (se multisede), ma senza assets se nuovo cliente multisede
      const basePayload = {
        ...data,
        indirizzo: indirizzoCompleto,
        citta: data.citta || '',
        cap: data.cap || '',
        is_pa: Boolean(data.is_pa),
        codice_sdi: data.codice_sdi || '',
        sedi: hasMultisede ? sedi : [],
        // Se è un nuovo cliente con multisede, salva prima solo le sedi (senza assets)
        // Gli assets verranno aggiunti dopo in un secondo update dopo aver ricaricato le sedi con gli ID
        assets_noleggio: (!isEdit && hasMultisede && hasNoleggio) ? [] : assetsToSend,
      ...
      console.log('Payload Step 1 (base + sedi):', JSON.stringify(basePayload, null, 2));

      let clienteId = id;
      
      if (isEdit) {
        // Modifica: salva tutto insieme
        await axios.put(`${getApiUrl()}/clienti/${id}`, basePayload);
        alert('Cliente aggiornato con successo!');
        navigate('/admin?tab=clienti');
      } else {
        // Nuovo cliente: gestione step
        if (hasMultisede && hasNoleggio && currentStep === 1) {
          // STEP 1: Crea cliente base con sedi (senza assets)
          const res = await axios.post(`${getApiUrl()}/clienti/`, basePayload);
          clienteId = res.data.id;
          setClienteIdStep1(clienteId);
          
          // Ricarica il cliente per ottenere le sedi con gli ID
          const clienteRes = await axios.get(`${getApiUrl()}/clienti/${clienteId}`);
          const clienteCreato = clienteRes.data;
          
          // Aggiorna lo stato delle sedi con gli ID dal backend
          if (clienteCreato.sedi && clienteCreato.sedi.length > 0) {
            const sediConId = clienteCreato.sedi.map((s: any) => ({
              id: s.id,
              nome_sede: s.nome_sede,
              indirizzo_completo: s.indirizzo_completo,
              citta: s.citta || '',
          ...
          // Passa allo step 2
          setCurrentStep(2);
          // Aggiorna l'URL per mantenere lo stato se si ricarica la pagina
          window.history.replaceState({}, '', `/nuovo-cliente/${clienteId}`);
          alert('Cliente creato con successo! Ora puoi aggiungere le macchine a noleggio.');
          navigate(`/nuovo-cliente/${clienteId}`, { replace: true });
          // Non navigare via, rimani sulla pagina per step 2
          return;
        } else if (hasMultisede && hasNoleggio && currentStep === 2 && clienteIdStep1) {
          // STEP 2: Aggiorna il cliente con gli assets (le sedi ora hanno gli ID corretti)
          clienteId = clienteIdStep1;
          
          // Ricarica il cliente per ottenere le sedi con gli ID (se non già caricate)
          const clienteRes = await axios.get(`${getApiUrl()}/clienti/${clienteId}`);
          const clienteCreato = clienteRes.data;
          ...
          let sediConId = sedi;
          if (clienteCreato.sedi && clienteCreato.sedi.length > 0) {
            sediConId = clienteCreato.sedi.map((s: any) => ({
              id: s.id,
              nome_sede: s.nome_sede,
              indirizzo_completo: s.indirizzo_completo,
              citta: s.citta || '',
          ...
          // Prepara gli assets con gli ID corretti delle sedi
          const assetsFinal = assetsToSend.map((asset: any) => {
            // Se l'asset ha un sede_id, verifica che corrisponda a una sede valida
            if (asset.sede_id !== null && asset.sede_id !== undefined && asset.sede_id !== '') {
              const sedeFound = sediConId.find((s: any) => s.id === asset.sede_id);
              if (!sedeFound) {
                // Se non trova la sede per ID, prova a cercare per nome o imposta a null
                asset.sede_id = null;
              }
            } else {
              asset.sede_id = null;
          ...
          // STEP 2: Aggiorna il cliente con gli assets
          const updatePayload = {
            ...data,
            indirizzo: indirizzoCompleto,
            citta: data.citta || '',
          ...
          await axios.put(`${getApiUrl()}/clienti/${clienteId}`, updatePayload);
          alert('Cliente creato con successo!');
          navigate('/admin?tab=clienti');
        } else {
          // Nuovo cliente senza multisede o senza noleggio: salva tutto insieme
          const res = await axios.post(`${getApiUrl()}/clienti/`, basePayload);
          alert('Cliente creato con successo!');
          navigate('/admin?tab=clienti');
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <main className="max-w-2xl mx-auto p-4 space-y-4">
        ...
        {/* Indicatore Step per nuovi clienti multisede con noleggio */}
        {!isEdit && hasMultisede && hasNoleggio && (
          <div className="mb-6 bg-white rounded-xl p-4 border border-gray-200">
            <div className="flex items-center justify-center gap-4">
              <div className={`flex items-center gap-2 ${currentStep >= 1 ? 'text-blue-600' : 'text-gray-400'}`}>
                ...
                <span className="font-semibold">Dati Cliente e Sedi</span>
              </div>
              ...
              <div className={`flex items-center gap-2 ${currentStep >= 2 ? 'text-blue-600' : 'text-gray-400'}`}>
                ...
                <span className="font-semibold">Macchine a Noleggio</span>
              </div>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <IOSCard>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                ...
                {!isEdit && hasMultisede && hasNoleggio && currentStep === 1 && (
            ...
            <IOSCard>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  {!isEdit && hasNoleggio && currentStep === 1 && (
                    <span className="ml-2 px-2 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full">
                      Step 1/2
                    </span>
                  )}
              ...
              {!isEdit && hasNoleggio && currentStep === 1 && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Step 1 di 2:</strong> Completa prima la creazione del cliente e delle sedi. 
                    Dopo aver salvato, potrai aggiungere le macchine a noleggio e assegnarle alle sedi corrette.
                  </p>
                </div>
              )}
          ...
          {/* Mostra sezione Macchine Noleggiate solo se:
              - È modifica, OPPURE
              - È nuovo cliente ma non multisede, OPPURE  
              - È nuovo cliente multisede ma siamo allo step 2 (dopo aver salvato le sedi) */}
          {hasNoleggio && (isEdit || !hasMultisede || (hasMultisede && currentStep === 2)) && (
            <IOSCard>
              {!isEdit && hasMultisede && currentStep === 2 && (
                <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-sm text-green-800">
                    <strong>Step 2 di 2:</strong> Ora puoi aggiungere le macchine a noleggio e assegnarle alle sedi corrette. 
                    Le sedi sono state create e sono disponibili nel menu a tendina.
                  </p>
                </div>
              )}
              <div className="flex items-center justify-between mb-6">
                    ...
                    <>
                      ...
                      {watch('has_multisede') && sedi.length > 0 && (
                        <div>
                          ...
                          <select
                            value={newAsset.sede_id ? String(newAsset.sede_id) : ''}
                            onChange={(e) => {
                              const val = e.target.value;
                              if (val === '' || val === '0') {
                                setNewAsset({ ...newAsset, sede_id: null });
                              } else {
                                // Se l'ID inizia con "temp_", è un indice temporaneo (non dovrebbe accadere nello step 2)
                                if (val.startsWith('temp_')) {
                                  const idx = parseInt(val.replace('temp_', ''));
                                  const sede = sedi[idx];
                                  setNewAsset({ ...newAsset, sede_id: sede?.id || null });
                                } else {
                                  const sedeId = parseInt(val);
                                  const sede = sedi.find(s => s.id === sedeId);
                                  setNewAsset({ ...newAsset, sede_id: sede?.id || sedeId });
                                }
                              }
                            }}
                            className="w-full bg-white border border-gray-200 rounded-xl py-3 pl-4 pr-4 text-sm focus:ring-2 focus:ring-blue-500 shadow-sm outline-none disabled:bg-gray-100 disabled:cursor-not-allowed"
                            disabled={!isEdit && hasMultisede && currentStep === 1}
                          >
                            <option value="">Sede Principale (Legale)</option>
                            {sedi.map((sede, idx) => {
                              // Usa l'ID se disponibile, altrimenti usa un indice temporaneo
                              const value = sede.id ? String(sede.id) : `temp_${idx}`;
                              return (
                                <option key={sede.id || `temp_${idx}`} value={value}>
                                  {sede.nome_sede} - {sede.indirizzo_completo}
                                </option>
                              );
                            })}
                          </select>
                          {!isEdit && hasMultisede && currentStep === 1 ? (
                            <p className="text-xs text-amber-600 mt-1 ml-1">
                              ⚠️ Completa prima la creazione del cliente e delle sedi per poter assegnare le macchine alle sedi.
                            </p>
                          ) : (
            ...
            >
              {isSaving ? (
                ...
                <>
                  ...
                  {isEdit 
                    ? 'Aggiorna' 
                    : (!hasMultisede || !hasNoleggio || currentStep === 2)
                      ? 'Crea Cliente'
                      : 'Salva e Continua'
                  }
                </>
