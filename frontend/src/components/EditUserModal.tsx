...
export default function EditUserModal({ user, onClose, onSave }: EditUserModalProps) {
  ...
  // Dichiarazione di isAdmin e isSuperAdmin all'inizio per evitare problemi di inizializzazione
  ...
  useEffect(() => {
    if (currentRuolo && (!user?.permessi || Object.keys(user.permessi || {}).length === 0)) {
      ...
      const defaultPerms = getDefaultPermessi(currentRuolo);
      // Se è Admin che modifica, forza can_edit_settings e can_delete_interventi a False
      if (isAdmin) {
        defaultPerms.can_edit_settings = false;
        defaultPerms.can_delete_interventi = false;
      }
      setPermessi(defaultPerms);
    }
  }, [currentRuolo, isAdmin]);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      // RESTRIZIONI PER ADMIN: forza can_edit_settings e can_delete_interventi a False
      const permessiFinali = { ...permessi };
      if (isAdmin) {
        permessiFinali.can_edit_settings = false;
        permessiFinali.can_delete_interventi = false;
      }
      
      // RESTRIZIONI PER ADMIN: non può creare SuperAdmin
      if (isCreating && isAdmin && data.ruolo === 'superadmin') {
        setError('Non puoi creare un utente SuperAdmin');
        setIsSaving(false);
        return;
      }
      
      // RESTRIZIONI PER ADMIN: non può cambiare ruolo in SuperAdmin
      if (!isCreating && isAdmin && data.ruolo === 'superadmin' && user?.ruolo !== 'superadmin') {
        setError('Non puoi cambiare il ruolo di un utente in SuperAdmin');
        setIsSaving(false);
        return;
      }
  ...
  // RESTRIZIONI PER ADMIN: non può creare SuperAdmin e non può abilitare certi permessi
  const canCreateSuperAdmin = isSuperAdmin;
  const canEditSettings = isSuperAdmin;
  const canDeleteInterventi = isSuperAdmin;
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <IOSCard>
              ...
              <IOSSelect
                ...
                {...register('ruolo', {
                  onChange: (e) => {
                    ...
                    if (!user?.permessi || Object.keys(user.permessi || {}).length === 0) {
                      const defaultPerms = getDefaultPermessi(nuovoRuolo);
                      // Se è Admin che modifica, forza can_edit_settings e can_delete_interventi a False
                      if (isAdmin) {
                        defaultPerms.can_edit_settings = false;
                        defaultPerms.can_delete_interventi = false;
                ...
                disabled={isAdmin && watch('ruolo') === 'superadmin' && isCreating}
                options={[
                  { value: 'superadmin', label: 'Super Admin (tutti i permessi)', disabled: !canCreateSuperAdmin && isCreating },
              ...
              />
              {isAdmin && !canCreateSuperAdmin && (
                <p className="text-xs text-red-500 ml-1 -mt-3 mb-2">
                  ⚠ Non puoi creare un utente SuperAdmin
                </p>
              )}
                    ...
                    <div key={categoria.categoria} className="border-b border-gray-100 last:border-0 pb-4 last:pb-0">
                      ...
                      <div className="space-y-2 ml-7">
                        {categoria.permessi.map((permesso) => {
                          // RESTRIZIONI PER ADMIN: disabilita can_edit_settings e can_delete_interventi
                          const isRestricted = isAdmin && (permesso.key === 'can_edit_settings' || permesso.key === 'can_delete_interventi');
                              ...
                              <div className="flex-1">
                                ...
                                {isDisabled && (
                                  <p className="text-xs text-red-500 mt-0.5">
                                    Solo SuperAdmin può abilitare questo permesso
                                  </p>
                                )}
                              </div>
                              <button
