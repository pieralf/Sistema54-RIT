...
async def _cors_middleware(request: Request, call_next):
    ...
    if request.method == "OPTIONS" and request.headers.get("access-control-request-method"):
        ...
        try:
            ...
            # Mantieni header CORS anche in caso di eccezioni non gestite
...
def delete_user(
    ...
    """
    Elimina un utente (solo SuperAdmin).

    Regole:
    - Tutti gli utenti sono eliminabili (hard delete) TRANNE il SuperAdmin di default
    ...
    - Prima della cancellazione, scollega eventuali riferimenti (tecnico_id) e rimuove i token di reset
    """
    default_sa_email = os.getenv("DEFAULT_SUPERADMIN_EMAIL", "admin@sistema54.it").strip().lower()

    user_obj = db.query(models.Utente).filter(models.Utente.id == user_id).first()
    if not user_obj:
        raise HTTPException(status_code=404, detail="Utente non trovato")
    ...
    if target_is_default_superadmin:
        ...
        # deve essere un altro superadmin (già garantito da require_superadmin)
    ...
    # 2) Protezione ultimo superadmin attivo (hard delete)
    if str(user_obj.ruolo) == "superadmin":
        active_superadmin_count = (
            db.query(models.Utente)
            .filter(models.Utente.ruolo == "superadmin", models.Utente.is_active == True)  # noqa: E712
            .count()
        )
        if active_superadmin_count <= 1:
            raise HTTPException(status_code=400, detail="Non puoi eliminare l'ultimo SuperAdmin attivo")

    self_delete = (user_obj.id == current_user.id)

    # 3) Scollega riferimenti FK noti (evita vincoli su tecnico_id)
    try:
        db.query(models.Intervento).filter(models.Intervento.tecnico_id == user_id).update(
            {models.Intervento.tecnico_id: None}, synchronize_session=False
        )
    except Exception:
        pass

    try:
        db.query(models.LetturaCopie).filter(models.LetturaCopie.tecnico_id == user_id).update(
            {models.LetturaCopie.tecnico_id: None}, synchronize_session=False
        )
    except Exception:
        pass

    # 4) Rimuove token reset/invito associati
    try:
    ...
    # 5) Hard delete
    email_deleted = user_obj.email
    try:
        db.delete(user_obj)
        db.commit()
        try:
            log_action(db, current_user, "DELETE", "utente", user_id, email_deleted)
        except Exception:
            pass
    except Exception as e:
        db.rollback()
        # Qui preferiamo un errore chiaro invece di soft-delete automatico
        raise HTTPException(
            status_code=400,
            detail=f"Impossibile eliminare l'utente per vincoli/relazioni nel DB: {type(e).__name__}",
        )
