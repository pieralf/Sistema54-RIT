...
def create_or_update_backup_target(
    ...
    # payload atteso:
    # {id?, name, kind: onedrive|gdrive|dropbox|smb, remote_path, enabled, config:{...}}

    # --- Normalizzazione input ---
    # 0) alias per compatibilità: il frontend può inviare "drive" per Google Drive
    ...
    if isinstance(cfg, str) and cfg.strip():
        try:
            ...
            raise HTTPException(status_code=400, detail="Config non valida: deve essere JSON")
