...
def register(
    ...
    db_user = models.Utente(
        ...
        permessi=(user_data.permessi if getattr(user_data, 'permessi', None) is not None else get_default_permessi(ruolo_enum.value)),
    )
    db.add(db_user)
    try:
        db.commit()
    ...
    base_url = os.getenv("FRONTEND_URL") or request.headers.get("origin") or request.headers.get("referer") or ""
    base_url = base_url.rstrip("/")
    if not base_url:
        base_url = "http://localhost:26081"
