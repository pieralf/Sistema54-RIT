...
# Print bcrypt/passlib versions to help diagnose hashing issues
try:
    import bcrypt as _bcrypt
    import passlib as _passlib
    print(f"ℹ️ bcrypt version: {getattr(_bcrypt, '__version__', 'unknown')}")
    print(f"ℹ️ passlib version: {getattr(_passlib, '__version__', 'unknown')}")
except Exception as _e:
    print("⚠️ Unable to read bcrypt/passlib versions:", _e)


# -------------------- CORS --------------------
# CORS robusto e "container-agnostic".
# - Se CORS_ORIGINS="*" (default): riflette sempre l'Origin della richiesta (compatibile con credenziali).
# - Se CORS_ORIGINS="http://a,http://b": permette solo le origini elencate.
#
# Nota: usiamo un middleware custom invece di CORSMiddleware per evitare i casi in cui,
# con credenziali + wildcard, gli header possano non essere aggiunti (preflight/errori inclusi).
_cors_raw = (os.getenv("CORS_ORIGINS", "*") or "*").strip()

_allowed_origins = None
if _cors_raw and _cors_raw not in ("*", "all", "ALL"):
...
async def _cors_middleware(request: Request, call_next):
    ...
    # Se non è una richiesta cross-origin, procedi normalmente
    if not origin:
        return await call_next(request)

    # Se abbiamo una whitelist, enforce
    if _allowed_origins is not None and origin not in _allowed_origins:
        # Risposta compatta: blocca esplicitamente (utile in produzione)
    ...
    if request.method == "OPTIONS" and request.headers.get("access-control-request-method"):
        ...
        try:
            ...
            # Log errore e mantieni header CORS anche in caso di eccezioni non gestite
            import traceback
            logger.error(f"Errore durante richiesta {request.method} {request.url.path}: {str(e)}")
            logger.error(f"Traceback completo:\n{traceback.format_exc()}")
            print(f"❌ Errore middleware CORS: {repr(e)}")
            traceback.print_exc()
            resp = JSONResponse(status_code=500, content={"detail": "Internal Server Error", "error": str(e) if DEBUG_ERRORS else None})

    # Riflette l'origin (necessario con credenziali)
    ...
    try:
        ...
        for ddt in ddts_aperti:
            if not ddt.data_ritiro:
                continue
            if not ddt.tecnico_assegnato_id:
                continue
            tecnico = db.query(models.Utente).filter(models.Utente.id == ddt.tecnico_assegnato_id).first()
            if not tecnico or not tecnico.email:
                continue

            stato_time = ddt.stato_updated_at or ddt.data_ritiro
            if not stato_time:
                continue
            giorni_aperto = (oggi - stato_time).days
            for soglia in soglie_giorni:
                key = (ddt.id, ddt.stato, soglia, stato_time.date().isoformat())
                if giorni_aperto >= soglia and key not in _ddt_reminder_sent:
                    subject = f"Promemoria DDT {ddt.numero_ddt} in stato {ddt.stato}"
                    body_html = f"""
                    <div style="font-family: Arial, sans-serif; color: #333;">
                        <p>Gentile {tecnico.nome_completo or 'Tecnico'},</p>
                        <p>il DDT <strong>{ddt.numero_ddt}</strong> è nello stato <strong>{ddt.stato}</strong> da <strong>{giorni_aperto} giorni</strong>.</p>
                        <p>Cliente: <strong>{ddt.cliente_ragione_sociale}</strong></p>
                        <p>Data ritiro: {ddt.data_ritiro.strftime('%d/%m/%Y') if ddt.data_ritiro else '-'}</p>
                    </div>
                    """
                    from_name = f"GIT - {settings.nome_azienda or 'GIT'} - DDT"
                    email_service.send_email(tecnico.email, subject, body_html, db=db, from_name=from_name)
...
# --- FUNZIONE PER AVVISI DDT DA ASSEGNARE ---
def check_ddt_assegnazioni():
    """Invia promemoria ogni 24h per DDT in magazzino da assegnare (modalità manuale)."""
    db = next(database.get_db())
    try:
        settings = db.query(models.ImpostazioniAzienda).first()
        if not settings:
            return
        if settings.ddt_assegnazione_modalita != "manual" or not settings.ddt_assegnazione_alert_abilitato:
            return

        smtp_config = email_service.get_smtp_config(db)
        if not smtp_config.get("username") or not smtp_config.get("password"):
            print("SMTP non configurato: promemoria assegnazioni DDT non inviati")
            return

        if not settings.email_responsabile_ddt:
            return

        pending_count = db.query(models.RitiroProdotto).filter(
            models.RitiroProdotto.deleted_at.is_(None),
            models.RitiroProdotto.stato == "in_magazzino",
            models.RitiroProdotto.assegnazione_stato == "da_assegnare"
        ).count()
        ...
        subject = "Promemoria DDT da assegnare ai tecnici"
        from_name = f"GIT - {settings.nome_azienda or 'GIT'} - DDT"

        for email_to in recipients:
            user = db.query(models.Utente).filter(models.Utente.email == email_to).first()
            destinatario_nome = user.nome_completo if user else email_to
            body_html = f"""
            <div style="font-family: Arial, sans-serif; color: #333;">
                <p>Gentile {destinatario_nome},</p>
                <p>Ti ricordo che hai <strong>{pending_count}</strong> DDT in sospeso da assegnare ai tecnici.</p>
                <p>Questo messaggio sarà inviato ogni 24 ore finché presenti DDT in sospeso da assegnare ai tecnici.</p>
                <p>Buon lavoro - <strong>GIT - Gestione Interventi Tecnici</strong>!</p>
            </div>
            """
            email_service.send_email(
                to_email=email_to,
        ...
        print(f"Errore invio promemoria assegnazioni DDT: {str(e)}")
        import traceback
        traceback.print_exc()
    finally:
        db.close()
...
def run_scheduled_backup(backup_type: str, target_ids: Optional[List[int]] = None):
    """Esegue un backup programmato per una destinazione specifica.
    
    Args:
        backup_type: 'local', 'nas', o 'cloud'
        target_ids: Lista di ID target per upload (solo per cloud)
    """
    db = next(database.get_db())
    try:
        settings = db.query(models.ImpostazioniAzienda).first()
        if not settings:
            print(f"[BACKUP SCHEDULER] Impostazioni non trovate, skip backup {backup_type}")
            return
        
        # Verifica se lo scheduler è abilitato per questo tipo
        schedule_enabled = False
        if backup_type == 'local':
            schedule_enabled = getattr(settings, 'backup_local_schedule_enabled', False)
        elif backup_type == 'nas':
            schedule_enabled = getattr(settings, 'backup_nas_schedule_enabled', False) and getattr(settings, 'backup_nas_enabled', False)
        elif backup_type == 'cloud':
            schedule_enabled = getattr(settings, 'backup_cloud_schedule_enabled', False) and getattr(settings, 'backup_cloud_enabled', False)
        
        if not schedule_enabled:
            print(f"[BACKUP SCHEDULER] Scheduler {backup_type} disabilitato, skip")
            return
        
        print(f"[BACKUP SCHEDULER] Avvio backup {backup_type}...")
        
        # Esegui backup
        result = create_backup(background=False, db=db, target_ids=target_ids)
        
        # Pulizia backup vecchi (retention)
        if backup_type == 'local':
            keep_count = getattr(settings, 'backup_local_keep_count', 10)
        elif backup_type == 'nas':
            keep_count = getattr(settings, 'backup_nas_keep_count', 10)
        else:  # cloud
            keep_count = getattr(settings, 'backup_cloud_keep_count', 10)
        
        # Mantieni solo gli ultimi N backup
        from .services.backup_service import list_backups, delete_backup
        backups = list_backups()
        if len(backups) > keep_count:
            # Ordina per data (più vecchi prima) e elimina quelli in eccesso
            backups_sorted = sorted(backups, key=lambda x: x['created_at'])
            to_delete = backups_sorted[:-keep_count]
            for backup in to_delete:
                try:
                    delete_backup(backup['id'])
                    print(f"[BACKUP SCHEDULER] Eliminato backup vecchio: {backup['id']}")
                except Exception as e:
                    print(f"[BACKUP SCHEDULER] Errore eliminazione backup {backup['id']}: {e}")
        
        print(f"[BACKUP SCHEDULER] Backup {backup_type} completato")
        
    except Exception as e:
        print(f"[BACKUP SCHEDULER] Errore backup {backup_type}: {e}")
        import traceback
        traceback.print_exc()
    finally:
        db.close()
...
def setup_backup_schedulers():
    """Configura gli scheduler per i backup automatici leggendo le impostazioni dal database."""
    db = next(database.get_db())
    try:
        logger.info("[BACKUP SCHEDULER] Inizio setup scheduler backup...")
        # Rimuovi scheduler backup esistenti prima di ricrearli
        try:
            scheduler.remove_job('backup_local')
            logger.debug("[BACKUP SCHEDULER] Rimosso job backup_local esistente")
        except Exception as e:
            logger.debug(f"[BACKUP SCHEDULER] Nessun job backup_local da rimuovere: {e}")
        try:
            scheduler.remove_job('backup_nas')
            logger.debug("[BACKUP SCHEDULER] Rimosso job backup_nas esistente")
        except Exception as e:
            logger.debug(f"[BACKUP SCHEDULER] Nessun job backup_nas da rimuovere: {e}")
        try:
            scheduler.remove_job('backup_cloud')
            logger.debug("[BACKUP SCHEDULER] Rimosso job backup_cloud esistente")
        except Exception as e:
            logger.debug(f"[BACKUP SCHEDULER] Nessun job backup_cloud da rimuovere: {e}")
        
        settings = db.query(models.ImpostazioniAzienda).first()
        if not settings:
            logger.warning("[BACKUP SCHEDULER] Impostazioni non trovate, scheduler backup non configurati")
            print("[BACKUP SCHEDULER] ⚠️ Impostazioni non trovate, scheduler backup non configurati")
            return
        
        local_enabled = getattr(settings, 'backup_local_schedule_enabled', None)
        nas_enabled = getattr(settings, 'backup_nas_schedule_enabled', None)
        cloud_enabled = getattr(settings, 'backup_cloud_schedule_enabled', None)
        
        logger.info(f"[BACKUP SCHEDULER] Impostazioni trovate: local_enabled={local_enabled} (type: {type(local_enabled)}), nas_enabled={nas_enabled} (type: {type(nas_enabled)}), cloud_enabled={cloud_enabled} (type: {type(cloud_enabled)})")
        print(f"[BACKUP SCHEDULER] Impostazioni: local={local_enabled}, nas={nas_enabled}, cloud={cloud_enabled}")
        
        # Backup locale
        if local_enabled is True:
            schedule_time = getattr(settings, 'backup_local_schedule_time', '02:00')
            try:
                ...
                scheduler.add_job(
                    ...
                    args=['local'],
                    id='backup_local',
                    name='Backup locale automatico',
                    replace_existing=True
                )
                logger.info(f"[BACKUP SCHEDULER] Configurato backup locale: ogni giorno alle {schedule_time}")
            except Exception as e:
                logger.error(f"[BACKUP SCHEDULER] Errore configurazione backup locale: {e}", exc_info=True)
        
        # Backup NAS
        nas_schedule_enabled = getattr(settings, 'backup_nas_schedule_enabled', None)
        nas_dest_enabled = getattr(settings, 'backup_nas_enabled', False)
        if nas_schedule_enabled is True and nas_dest_enabled:
            schedule_time = getattr(settings, 'backup_nas_schedule_time', '02:00')
            try:
                hour, minute = map(int, schedule_time.split(':'))
                scheduler.add_job(
                    run_scheduled_backup,
                    ...
                    args=['nas'],
                    id='backup_nas',
                    name='Backup NAS automatico',
                    replace_existing=True
                )
                logger.info(f"[BACKUP SCHEDULER] Configurato backup NAS: ogni giorno alle {schedule_time}")
            except Exception as e:
                logger.error(f"[BACKUP SCHEDULER] Errore configurazione backup NAS: {e}", exc_info=True)
        
        # Backup Cloud
        cloud_schedule_enabled = getattr(settings, 'backup_cloud_schedule_enabled', None)
        # Verifica se ci sono target cloud/network configurati (non serve backup_cloud_enabled)
        cloud_targets = db.query(models.BackupTarget).filter(
            models.BackupTarget.enabled.is_(True),
            models.BackupTarget.kind.in_(['onedrive', 'gdrive', 'dropbox', 'smb', 'ftp', 'sftp'])
        ).all()
        has_cloud_targets = len(cloud_targets) > 0
        
        if cloud_schedule_enabled is True and has_cloud_targets:
            schedule_time = getattr(settings, 'backup_cloud_schedule_time', '02:00')
            target_ids = [t.id for t in cloud_targets] if cloud_targets else None
            ...
            try:
                ...
                scheduler.add_job(
                    run_scheduled_backup,
                    trigger=CronTrigger(hour=hour, minute=minute),
                    args=['cloud', target_ids],
                    id='backup_cloud',
                    name='Backup cloud automatico',
                    replace_existing=True
                )
                logger.info(f"[BACKUP SCHEDULER] Configurato backup cloud: ogni giorno alle {schedule_time} (targets: {len(cloud_targets) if cloud_targets else 0})")
                print(f"[BACKUP SCHEDULER] ✅ Backup cloud schedulato: ogni giorno alle {schedule_time}")
            except Exception as e:
                logger.error(f"[BACKUP SCHEDULER] Errore configurazione backup cloud: {e}", exc_info=True)
        elif cloud_schedule_enabled is True and not has_cloud_targets:
            logger.warning("[BACKUP SCHEDULER] Scheduler cloud abilitato ma nessun target cloud configurato. Scheduler non configurato.")
            print("[BACKUP SCHEDULER] ⚠️ Scheduler cloud abilitato ma nessun target cloud configurato")
                
    except Exception as e:
        logger.error(f"[BACKUP SCHEDULER] Errore setup scheduler backup: {e}", exc_info=True)
    finally:
        db.close()
...
scheduler.add_job(
    check_ddt_assegnazioni,
    trigger=CronTrigger(hour=9, minute=0),  # Ogni giorno alle 9:00
    id='check_ddt_assegnazioni',
    name='Promemoria DDT da assegnare',
    replace_existing=True
)
...
print("  - DDT da assegnare: ogni giorno alle 9:00")

# Setup backup schedulers dopo l'avvio
try:
    setup_backup_schedulers()
except Exception as e:
    logger.error(f"Errore setup backup schedulers all'avvio: {e}", exc_info=True)
    print(f"❌ Errore setup backup schedulers all'avvio: {e}")
    import traceback
    traceback.print_exc()
...
def send_ddt_created_email(db: Session, ddt: models.RitiroProdotto, cliente: models.Cliente, settings: Optional[models.ImpostazioniAzienda]):
    """Invia email creazione DDT a cliente e responsabili DDT."""
    smtp_config = email_service.get_smtp_config(db)
    if not smtp_config.get("username") or not smtp_config.get("password"):
        print("SMTP non configurato: email creazione DDT non inviata")
        return

    recipients = set()
    if cliente.email_amministrazione:
        recipients.add(cliente.email_amministrazione)
    if settings and settings.email_responsabile_ddt:
        recipients.update(parse_email_list(settings.email_responsabile_ddt))
    if not recipients:
        return

    subject = f"Nuovo DDT {ddt.numero_ddt} creato"
    from_name = f"GIT - {settings.nome_azienda or 'GIT'} - DDT" if settings else "GIT - DDT"
    body_html = f"""
    <div style="font-family: Arial, sans-serif; color: #333;">
        <p>È stato creato un nuovo DDT <strong>{ddt.numero_ddt}</strong>.</p>
        <p>Cliente: <strong>{ddt.cliente_ragione_sociale}</strong></p>
...
def send_ddt_assignment_email(db: Session, ddt: models.RitiroProdotto, tecnico: models.Utente, settings: Optional[models.ImpostazioniAzienda]):
    """Invia email al tecnico assegnatario in attesa di accettazione."""
    if not tecnico.email:
        return
    smtp_config = email_service.get_smtp_config(db)
    if not smtp_config.get("username") or not smtp_config.get("password"):
        print("SMTP non configurato: email assegnazione DDT non inviata")
        return

    subject = f"Assegnazione DDT {ddt.numero_ddt} in attesa di accettazione"
    from_name = f"GIT - {settings.nome_azienda or 'GIT'} - DDT" if settings else "GIT - DDT"
    body_html = f"""
    <div style="font-family: Arial, sans-serif; color: #333;">
        <p>Gentile {tecnico.nome_completo or 'Tecnico'},</p>
        <p>ti è stato assegnato il DDT <strong>{ddt.numero_ddt}</strong> ed è in attesa di accettazione.</p>
        <p>Cliente: <strong>{ddt.cliente_ragione_sociale}</strong></p>
    </div>
    """
    email_service.send_email(
...
def select_available_tecnico(db: Session, settings: Optional[models.ImpostazioniAzienda]) -> Optional[models.Utente]:
    """Seleziona un tecnico disponibile con round-robin e priorità su carichi bassi."""
    tecnici = db.query(models.Utente).filter(
        models.Utente.ruolo == models.RuoloUtente.TECNICO,
        models.Utente.is_active.is_(True),
        models.Utente.deleted_at.is_(None)
    ).order_by(models.Utente.id.asc()).all()
    if not tecnici:
        return None

    advanced = (settings.configurazioni_avanzate or {}) if settings else {}
    strategy = advanced.get("ddt_auto_strategy", "round_robin_weighted")
    prioritize_assigned = advanced.get("ddt_auto_prioritize_assigned", True)
    prioritize_worked = advanced.get("ddt_auto_prioritize_worked", True)
    last_assigned_id = advanced.get("ddt_auto_last_tecnico_id")

    if strategy == "round_robin_only":
        if last_assigned_id:
            ids = [t.id for t in tecnici]
            if last_assigned_id in ids:
                idx = ids.index(last_assigned_id)
                return tecnici[(idx + 1) % len(tecnici)]
        return tecnici[0]

    tecnico_stats = []
    for tecnico in tecnici:
        active_count = db.query(models.RitiroProdotto).filter(
            models.RitiroProdotto.deleted_at.is_(None),
            models.RitiroProdotto.tecnico_assegnato_id == tecnico.id,
            models.RitiroProdotto.stato.in_(["in_magazzino", "in_riparazione"])
        ).count()
        worked_count = db.query(models.RitiroProdotto).filter(
            models.RitiroProdotto.deleted_at.is_(None),
            models.RitiroProdotto.tecnico_assegnato_id == tecnico.id,
            models.RitiroProdotto.stato.in_(["riparato", "consegnato", "scartato"])
        ).count()
        key_assigned = active_count if prioritize_assigned else 0
        key_worked = worked_count if prioritize_worked else 0
        tecnico_stats.append((key_assigned, key_worked, tecnico))

    tecnico_stats.sort(key=lambda x: (x[0], x[1]))
    best_assigned = tecnico_stats[0][0]
    best_worked = tecnico_stats[0][1]
    candidates = [t for a, w, t in tecnico_stats if a == best_assigned and w == best_worked]
    if not candidates:
        return tecnici[0]
    ...
    if last_assigned_id:
        ...
        if last_assigned_id in candidate_ids:
            idx = candidate_ids.index(last_assigned_id)
...
# (rotte spostate in router) 747-986
...
def list_users(
    ...
    """Lista tutti gli utenti (solo admin). Esclude gli utenti eliminati per default."""
...
def list_deleted_users(
    ...
    """Lista tutti gli utenti eliminati (soft delete). Solo admin."""
    return db.query(models.Utente).filter(
        models.Utente.deleted_at.isnot(None)
    ).order_by(models.Utente.deleted_at.desc()).offset(skip).limit(limit).all()
...
def update_user(user_id: int, user_update: schemas.UserUpdate, request: Request, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.require_admin)):
    ...
    # RESTRIZIONI PER ADMIN: non può abilitare can_edit_settings e can_delete_interventi
    if current_user.ruolo != models.RuoloUtente.SUPERADMIN and "permessi" in update_data:
        if isinstance(update_data["permessi"], dict):
            # Forza can_edit_settings e can_delete_interventi a False per Admin
            update_data["permessi"] = update_data["permessi"].copy()
            update_data["permessi"]["can_edit_settings"] = False
            update_data["permessi"]["can_delete_interventi"] = False
    ...
    if "permessi" not in update_data and (not db_user.permessi or db_user.permessi == {}):
        permessi_default = get_default_permessi(db_user.ruolo.value)
        # Se è Admin che modifica, forza i permessi vietati a False
        if current_user.ruolo != models.RuoloUtente.SUPERADMIN:
            permessi_default["can_edit_settings"] = False
            permessi_default["can_delete_interventi"] = False
        update_data["permessi"] = permessi_default
...
@app.delete("/api/users/{user_id}", tags=["Utenti"])
def delete_user(
    user_id: int,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: models.Utente = Depends(auth.require_admin),  # Admin e SuperAdmin
):
    """
    Elimina un utente con soft delete (Admin e SuperAdmin).

    Regole:
    - Tutti gli utenti sono eliminabili (soft delete) TRANNE il SuperAdmin di default
    - Il SuperAdmin di default NON è eliminabile: può essere solo DISATTIVATO
      * solo da un ALTRO SuperAdmin (non self)
    - Non consente l'eliminazione dell'ultimo SuperAdmin attivo
    ...
    user_obj = db.query(models.Utente).filter(
        ...
        models.Utente.deleted_at.is_(None)  # Solo utenti non già eliminati
    ).first()
    if not user_obj:
        raise HTTPException(status_code=404, detail="Utente non trovato")

    target_is_default_superadmin = (
        (str(user_obj.ruolo) == "superadmin") and ((user_obj.email or "").strip().lower() == default_sa_email)
    )

    # 1) SuperAdmin di default: solo disattivazione, mai delete
    if target_is_default_superadmin:
        if current_user.id == user_obj.id:
            raise HTTPException(status_code=400, detail="Non puoi disattivare il SuperAdmin di default da solo")
        # deve essere un altro superadmin
        if current_user.ruolo != models.RuoloUtente.SUPERADMIN:
            raise HTTPException(status_code=403, detail="Solo un SuperAdmin può disattivare il SuperAdmin di default")
    ...
    # 2) Un Admin non può eliminare SuperAdmin
    if current_user.ruolo != models.RuoloUtente.SUPERADMIN and str(user_obj.ruolo) == "superadmin":
        raise HTTPException(status_code=403, detail="Non puoi eliminare un SuperAdmin")

    # 3) Protezione ultimo superadmin attivo
    if str(user_obj.ruolo) == "superadmin":
        active_superadmin_count = (
            db.query(models.Utente)
            .filter(
                models.Utente.ruolo == "superadmin", 
                models.Utente.is_active == True,  # noqa: E712
                models.Utente.deleted_at.is_(None)  # Escludi eliminati
            )
            .count()
        )
        if active_superadmin_count <= 1:
            raise HTTPException(status_code=400, detail="Non puoi eliminare l'ultimo SuperAdmin attivo")

    # 4) Applica soft delete (preserva lo storico)
    ...
    try:
        ...
        log_action(
            ...
            entity_name=user_obj.email,
            changes={
                "deleted_at": {"old": None, "new": str(user_obj.deleted_at)},
                "email": user_obj.email,
                "ruolo": str(user_obj.ruolo)
            },
            ip_address=get_client_ip(request),
        )
        ...
        return {
            "ok": True,
            "message": "Utente eliminato con successo (soft delete)",
            "user_id": user_id,
            "email": user_obj.email,
            "storico_preservato": True
        }
        
    except Exception as e:
        db.rollback()
        logger.error(f"Errore eliminazione utente {user_id}: {str(e)}", exc_info=True)
    ...
    try:
        log_action(
            ...
            entity_type="utente",
            entity_id=user_id,
            entity_name=(user_obj.email if 'user_obj' in locals() else None),
            changes=None,
            ip_address=get_client_ip(request),
        )
    except Exception:
    ...
    return {"ok": True, "self_deleted": self_delete}
...
def create_cliente(cliente: schemas.ClienteCreate, request: Request, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    ...
    if cliente.p_iva:
        is_valid, error_msg = validate_partita_iva(cliente.p_iva)
        if not is_valid:
            raise HTTPException(status_code=400, detail=f"Partita IVA non valida: {error_msg}")
    
    # 2. Validazione Codice Fiscale
    if cliente.codice_fiscale:
        is_valid, error_msg = validate_codice_fiscale(cliente.codice_fiscale)
        if not is_valid:
            raise HTTPException(status_code=400, detail=f"Codice Fiscale non valido: {error_msg}")
    
    # 3. Controllo Duplicati (Solo se PIVA o CF sono forniti E non vuoti)
    ...
    if p_iva_valida or cf_valido:
        filters = []
        if p_iva_valida:
            # Confronta solo con record che hanno P.IVA non vuota/null e corrispondente (escludendo cancellati)
            filters.append(
                and_(
                    models.Cliente.p_iva.isnot(None),
                    models.Cliente.p_iva != "",
                    models.Cliente.p_iva == cliente.p_iva.strip(),
                    models.Cliente.deleted_at.is_(None)  # Escludi clienti cancellati
                )
            )
        if cf_valido:
            # Confronta solo con record che hanno CF non vuoto/null e corrispondente (escludendo cancellati)
            filters.append(
                and_(
    ...
    try:
        ...
        # IMPORTANTE: Le sedi devono essere create PRIMA degli assets per permettere la validazione delle foreign key
        sedi_creati = {}  # Mappa nome_sede -> db_sede per riferimento rapido
        ...
        if cliente.sede_legale_operativa:
            ...
            if not sede_legale_esistente:
                ...
                db.flush()  # Flush per ottenere l'ID della sede
                sedi_creati["Sede Legale/Centrale"] = db_sede_legale
        ...
        if cliente.has_multisede and cliente.sedi:
            for sede in cliente.sedi:
                ...
                db.add(db_sede)
                db.flush()  # Flush per ottenere l'ID della sede
                sedi_creati[sede.nome_sede] = db_sede

        # 4. Aggiunta Assets (se presenti)
        # IMPORTANTE: Validare che le sedi referenziate esistano e appartengano al cliente
        if hasattr(cliente, 'assets_noleggio') and cliente.assets_noleggio:
            # Carica tutte le sedi del cliente per validazione
            sedi_cliente_ids = {s.id for s in db.query(models.SedeCliente).filter(models.SedeCliente.cliente_id == db_cliente.id).all()}
            
            for asset in cliente.assets_noleggio:
                asset_dict = asset.model_dump()
                sede_id = asset_dict.get('sede_id')
                
                # Normalizza sede_id: null se 0 o non valido
                if sede_id == 0 or sede_id == '0' or sede_id is None:
                    asset_dict['sede_id'] = None
                else:
                    # Valida che la sede esista e appartenga al cliente
                    sede_id_int = int(sede_id) if isinstance(sede_id, str) else sede_id
                    if sede_id_int not in sedi_cliente_ids:
                        # Cerca per nome_sede se fornito (per compatibilità con frontend)
                        sede_nome = asset_dict.get('sede_nome')  # Campo temporaneo che potrebbe essere inviato dal frontend
                        if sede_nome and sede_nome in sedi_creati:
                            asset_dict['sede_id'] = sedi_creati[sede_nome].id
                            asset_dict.pop('sede_nome', None)  # Rimuovi campo temporaneo
                        else:
                            raise ValueError(f"Sede con ID {sede_id_int} non trovata o non appartiene al cliente")
                    else:
                        asset_dict['sede_id'] = sede_id_int
                
                # Rimuovi eventuali campi non presenti nel modello
                asset_dict.pop('sede_nome', None)
        ...
        db.rollback()
        logger.warning(f"Errore validazione creazione cliente: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        db.rollback()
        logger.error(f"Errore DB creazione cliente: {str(e)}", exc_info=True)
...
def get_cliente(cliente_id: int, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    ...
    # Se sede_legale_operativa è True ma la sede legale non esiste, creala automaticamente
    if db_cliente.sede_legale_operativa:
        sede_legale_esistente = db.query(models.SedeCliente).filter(
            models.SedeCliente.cliente_id == cliente_id,
            models.SedeCliente.nome_sede == "Sede Legale/Centrale"
        ).first()
        ...
        if not sede_legale_esistente:
            ...
            db.add(db_sede_legale)
            db.commit()
            db.refresh(db_cliente)  # Ricarica il cliente per includere la nuova sede
...
@app.delete("/clienti/{cliente_id}", tags=["Clienti"])
def delete_cliente(
    cliente_id: int,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: models.Utente = Depends(auth.require_admin)  # Solo Admin e SuperAdmin
):
    """
    Elimina un cliente con soft delete (preserva lo storico).
    
    Regole:
    - Se il cliente ha movimentazioni (interventi, letture copie, etc.), viene applicato solo soft delete
    - Lo storico viene sempre preservato
    - Solo Admin e SuperAdmin possono eliminare clienti
    """
    db_cliente = db.query(models.Cliente).filter(
        models.Cliente.id == cliente_id,
        models.Cliente.deleted_at.is_(None)  # Solo clienti non già cancellati
    ).first()
    
    if not db_cliente:
        raise HTTPException(status_code=404, detail="Cliente non trovato")
    
    # Verifica se ci sono movimentazioni (interventi)
    interventi_count = db.query(models.Intervento).filter(models.Intervento.cliente_id == cliente_id).count()
    
    # Verifica se ci sono letture copie associate agli assets del cliente
    letture_count = 0
    if db_cliente.assets_noleggio:
        asset_ids = [asset.id for asset in db_cliente.assets_noleggio]
        if asset_ids:
            letture_count = db.query(models.LetturaCopie).filter(
                models.LetturaCopie.asset_id.in_(asset_ids)
            ).count()
    
    # Applica soft delete (sempre, anche se non ci sono movimentazioni per preservare i dati)
    db_cliente.deleted_at = datetime.now()
    ...
    try:
        ...
        log_action(
            ...
            entity_name=db_cliente.ragione_sociale,
            changes={
                "deleted_at": {"old": None, "new": str(db_cliente.deleted_at)},
                "interventi_count": interventi_count,
                "letture_count": letture_count
            },
            ip_address=get_client_ip(request)
        )
        ...
        return {
            "ok": True,
            "message": "Cliente eliminato con successo (soft delete)",
            "cliente_id": cliente_id,
        ...
        db.rollback()
        logger.error(f"Errore eliminazione cliente {cliente_id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Errore durante l'eliminazione: {str(e)}")
        ...
        if hasattr(cliente, 'assets_noleggio') and cliente.assets_noleggio is not None and len(cliente.assets_noleggio) > 0:
            ...
            for asset in cliente.assets_noleggio:
                ...
                if sede_id == 0 or sede_id == '0' or sede_id is None:
                    ...
                    # Valida che la sede esista e appartenga al cliente
                    sede_id_int = int(sede_id) if isinstance(sede_id, str) else sede_id
                    # Verifica che la sede appartenga al cliente
                    sede_valida = db.query(models.SedeCliente).filter(
                        models.SedeCliente.id == sede_id_int,
                        models.SedeCliente.cliente_id == cliente_id
                    ).first()
                    if not sede_valida:
                        raise ValueError(f"Sede con ID {sede_id_int} non trovata o non appartiene al cliente")
                    asset_dict['sede_id'] = sede_id_int
                    sedi_referenziate_da_nuovi_assets.add(sede_id_int)
...
def create_prodotto(prodotto: schemas.ProdottoCreate, request: Request, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    existing = db.query(models.ProdottoMagazzino).filter(
        models.ProdottoMagazzino.codice_articolo == prodotto.codice_articolo,
        models.ProdottoMagazzino.deleted_at.is_(None)  # Escludi prodotti cancellati
    ).first()
    if existing:
...
def read_magazzino(q: str = "", skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    query = db.query(models.ProdottoMagazzino).filter(models.ProdottoMagazzino.deleted_at.is_(None))  # Escludi prodotti cancellati
    if q:
        search = f"%{q}%"
        query = query.filter(
            or_(
...
def update_prodotto(prodotto_id: int, prodotto: schemas.ProdottoUpdate, request: Request, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    db_prodotto = db.query(models.ProdottoMagazzino).filter(
    ...
    # Verifica che codice_articolo non sia duplicato (se viene aggiornato, escludendo cancellati)
...
@app.delete("/magazzino/{prodotto_id}", tags=["Magazzino"])
def delete_prodotto(
    prodotto_id: int,
    request: Request,
    db: Session = Depends(database.get_db),
    ...
    """
    Elimina un prodotto con soft delete (preserva lo storico).
    
    Regole:
    - Se il prodotto ha movimentazioni (movimenti_ricambi), viene applicato solo soft delete
    - Lo storico viene sempre preservato
    - Solo Admin e SuperAdmin possono eliminare prodotti
    """
    db_prodotto = db.query(models.ProdottoMagazzino).filter(
        models.ProdottoMagazzino.id == prodotto_id,
        models.ProdottoMagazzino.deleted_at.is_(None)  # Solo prodotti non già cancellati
    ).first()
    
    if not db_prodotto:
        raise HTTPException(status_code=404, detail="Prodotto non trovato")
    
    # Verifica se ci sono movimentazioni (movimenti_ricambi)
    movimenti_count = db.query(models.MovimentoRicambio).filter(
        models.MovimentoRicambio.prodotto_id == prodotto_id
    ).count()
    
    # Applica soft delete (sempre, anche se non ci sono movimentazioni per preservare i dati)
    db_prodotto.deleted_at = datetime.now()
    ...
    try:
        ...
        log_action(
            ...
            entity_name=db_prodotto.codice_articolo or db_prodotto.descrizione,
            changes={
                "deleted_at": {"old": None, "new": str(db_prodotto.deleted_at)},
        ...
        return {
            "ok": True,
            "message": "Prodotto eliminato con successo (soft delete)",
        ...
        db.rollback()
        logger.error(f"Errore eliminazione prodotto {prodotto_id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Errore durante l'eliminazione: {str(e)}")
...
def read_intervento(intervento_id: int, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    intervento = db.query(models.Intervento).filter(
        models.Intervento.id == intervento_id,
        models.Intervento.deleted_at.is_(None)  # Escludi interventi eliminati
    ).first()
...
def update_intervento(
    ...
    db_intervento = db.query(models.Intervento).filter(
        models.Intervento.id == intervento_id,
        models.Intervento.deleted_at.is_(None)  # Escludi interventi eliminati
    ).first()
...
@app.delete("/interventi/{intervento_id}", tags=["R.I.T."])
def delete_intervento(
    ...
    """
    Elimina un intervento (RIT) con soft delete (preserva lo storico).
    
    Regole:
    - Solo Admin e SuperAdmin possono eliminare interventi
    - Lo storico viene sempre preservato (soft delete)
    """
    db_intervento = db.query(models.Intervento).filter(
        models.Intervento.id == intervento_id,
        models.Intervento.deleted_at.is_(None)  # Solo interventi non già cancellati
    ).first()
    
    if not db_intervento:
        raise HTTPException(status_code=404, detail="Intervento non trovato")
    
    # Verifica permessi: solo Admin e SuperAdmin possono eliminare
    # Il require_admin già garantisce questo, ma aggiungiamo controllo extra
    if current_user.ruolo not in [models.RuoloUtente.ADMIN, models.RuoloUtente.SUPERADMIN]:
        raise HTTPException(status_code=403, detail="Solo Admin e SuperAdmin possono eliminare interventi")
    
    # Applica soft delete (preserva lo storico)
    db_intervento.deleted_at = datetime.now()
    ...
    try:
        ...
        log_action(
            ...
            entity_name=db_intervento.numero_relazione,
            changes={
                "deleted_at": {"old": None, "new": str(db_intervento.deleted_at)},
                "numero_relazione": db_intervento.numero_relazione,
                "cliente_ragione_sociale": db_intervento.cliente_ragione_sociale
            },
            ip_address=get_client_ip(request)
        )
        ...
        return {
            "ok": True,
            "message": "Intervento eliminato con successo (soft delete)",
        ...
        db.rollback()
        logger.error(f"Errore eliminazione intervento {intervento_id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Errore durante l'eliminazione: {str(e)}")
...
def download_pdf_rit(intervento_id: int, db: Session = Depends(database.get_db), current_user: models.Utente = Depends(auth.get_current_active_user)):
    rit = db.query(models.Intervento).filter(
        models.Intervento.id == intervento_id,
        models.Intervento.deleted_at.is_(None)  # Escludi interventi eliminati
    ).first()
...
# (rotte spostate in router) 2703-2736
...
def create_ddt(
    ...
    try:
        ...
        ddt_data.update({
            ...
            "stato_updated_at": datetime.now()
        })

        # Gestione assegnazione tecnico post-ritiro
        settings = db.query(models.ImpostazioniAzienda).first()
        assegnazione_modalita = settings.ddt_assegnazione_modalita if settings else "manual"
        ddt_data["assegnazione_stato"] = "da_assegnare"
        ddt_data["tecnico_assegnato_id"] = None
        ddt_data["tecnico_assegnazione_pending_id"] = None
        ddt_data["assegnazioni_log"] = []
        ddt_data["note_log"] = []

        if assegnazione_modalita == "auto":
            tecnico_auto = select_available_tecnico(db, settings)
            if tecnico_auto:
                ddt_data["tecnico_assegnazione_pending_id"] = tecnico_auto.id
                ddt_data["assegnazione_stato"] = "in_attesa_accettazione"
        ...
        if assegnazione_modalita == "auto" and settings and tecnico_auto:
            settings.configurazioni_avanzate = {
                **(settings.configurazioni_avanzate or {}),
                "ddt_auto_last_tecnico_id": tecnico_auto.id
            }
            db.add(settings)
            db.commit()

        # Email creazione DDT a cliente e responsabili
        try:
            send_ddt_created_email(db, db_ddt, db_cliente, settings)
        except Exception as e:
            logger.warning(f"Errore invio email creazione DDT: {e}")

        # Email assegnazione automatica al tecnico
        if assegnazione_modalita == "auto" and tecnico_auto:
            try:
                send_ddt_assignment_email(db, db_ddt, tecnico_auto, settings)
            except Exception as e:
                logger.warning(f"Errore invio email assegnazione DDT: {e}")
        ...
        if db_ddt.tecnico_assegnato_rel:
...
def list_ddt(
    ...
    for ddt in ddt_list:
        ...
        if ddt.tecnico_assegnato_rel:
            ddt_dict["tecnico_assegnato_nome"] = ddt.tecnico_assegnato_rel.nome_completo
        if ddt.tecnico_pending_rel:
            ddt_dict["tecnico_assegnazione_pending_nome"] = ddt.tecnico_pending_rel.nome_completo
...
@app.get("/ddt/pending-accept-count", tags=["DDT"])
def get_ddt_pending_accept_count(
    db: Session = Depends(database.get_db),
    current_user: models.Utente = Depends(auth.get_current_active_user)
):
    """Conta i DDT in attesa di accettazione per il tecnico loggato."""
    if current_user.ruolo != "tecnico":
        return {"count": 0}

    count = db.query(models.RitiroProdotto).filter(
        models.RitiroProdotto.deleted_at.is_(None),
...
def update_ddt(
    ...
    try:
        ...
        # Log note con firma tecnico (se cambiate)
        note_entries = []
        if "note" in update_data and update_data.get("note") != db_ddt.note:
            note_entries.append({
                "campo": "note",
                "valore": update_data.get("note") or "",
                "tecnico_id": current_user.id,
                "tecnico_nome": current_user.nome_completo,
                "timestamp": datetime.now().isoformat()
            })
        if "note_lavoro" in update_data and update_data.get("note_lavoro") != db_ddt.note_lavoro:
            note_entries.append({
                "campo": "note_lavoro",
                "valore": update_data.get("note_lavoro") or "",
                "tecnico_id": current_user.id,
                "tecnico_nome": current_user.nome_completo,
                "timestamp": datetime.now().isoformat()
            })
        if note_entries:
            db_ddt.note_log = (db_ddt.note_log or []) + note_entries

        # Aggiorna timestamp ultimo stato se cambia
        if "stato" in update_data and update_data.get("stato") and update_data.get("stato") != db_ddt.stato:
            db_ddt.stato_updated_at = datetime.now()
...
@app.post("/ddt/{ddt_id}/assign", response_model=schemas.RitiroProdottoResponse, tags=["DDT"])
def assign_ddt(
    ddt_id: int,
    payload: schemas.DdtAssignRequest,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: models.Utente = Depends(auth.require_admin)
):
    """Assegna (o riassegna) un DDT a un tecnico."""
    db_ddt = db.query(models.RitiroProdotto).filter(
        models.RitiroProdotto.id == ddt_id,
        models.RitiroProdotto.deleted_at.is_(None)
    ).first()
    ...
    tecnico = db.query(models.Utente).filter(
        models.Utente.id == payload.tecnico_id,
        models.Utente.deleted_at.is_(None),
        models.Utente.is_active.is_(True),
        models.Utente.ruolo == models.RuoloUtente.TECNICO
    ).first()
    if not tecnico:
        raise HTTPException(status_code=400, detail="Tecnico non valido o non disponibile")

    is_transfer = db_ddt.tecnico_assegnato_id and db_ddt.tecnico_assegnato_id != tecnico.id
    db_ddt.tecnico_assegnazione_pending_id = tecnico.id
    db_ddt.assegnazione_stato = "trasferimento_in_attesa" if is_transfer else "in_attesa_accettazione"

    db_ddt.assegnazioni_log = (db_ddt.assegnazioni_log or []) + [{
        "azione": "trasferimento_richiesto" if is_transfer else "assegnazione_richiesta",
        "da_tecnico_id": db_ddt.tecnico_assegnato_id,
        "a_tecnico_id": tecnico.id,
    ...
    try:
        settings = db.query(models.ImpostazioniAzienda).first()
        send_ddt_assignment_email(db, db_ddt, tecnico, settings)
    except Exception as e:
        logger.warning(f"Errore invio email assegnazione DDT: {e}")
...
@app.post("/ddt/{ddt_id}/accept-assignment", response_model=schemas.RitiroProdottoResponse, tags=["DDT"])
def accept_ddt_assignment(
    ddt_id: int,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: models.Utente = Depends(auth.get_current_active_user)
):
    """Accetta un'assegnazione/trasferimento DDT."""
    db_ddt = db.query(models.RitiroProdotto).filter(
        models.RitiroProdotto.id == ddt_id,
        models.RitiroProdotto.deleted_at.is_(None)
    ).first()
    if not db_ddt:
        raise HTTPException(status_code=404, detail="DDT non trovato")
    if db_ddt.tecnico_assegnazione_pending_id != current_user.id:
        raise HTTPException(status_code=403, detail="Non sei autorizzato ad accettare questa assegnazione")

    db_ddt.tecnico_assegnato_id = current_user.id
    db_ddt.tecnico_assegnazione_pending_id = None
    db_ddt.assegnazione_stato = "assegnato"
    db_ddt.stato = "in_riparazione"
    db_ddt.stato_updated_at = datetime.now()

    db_ddt.assegnazioni_log = (db_ddt.assegnazioni_log or []) + [{
        "azione": "assegnazione_accettata",
        "a_tecnico_id": current_user.id,
        "by_user_id": current_user.id,
        "by_user_nome": current_user.nome_completo,
        "timestamp": datetime.now().isoformat()
    }]
...
@app.post("/ddt/{ddt_id}/reject-assignment", response_model=schemas.RitiroProdottoResponse, tags=["DDT"])
def reject_ddt_assignment(
    ddt_id: int,
    request: Request,
    db: Session = Depends(database.get_db),
    current_user: models.Utente = Depends(auth.get_current_active_user)
):
    """Rifiuta un'assegnazione/trasferimento DDT."""
    db_ddt = db.query(models.RitiroProdotto).filter(
        models.RitiroProdotto.id == ddt_id,
        models.RitiroProdotto.deleted_at.is_(None)
    ).first()
    if not db_ddt:
        raise HTTPException(status_code=404, detail="DDT non trovato")
    if db_ddt.tecnico_assegnazione_pending_id != current_user.id:
        raise HTTPException(status_code=403, detail="Non sei autorizzato a rifiutare questa assegnazione")

    is_transfer = db_ddt.assegnazione_stato == "trasferimento_in_attesa"
    db_ddt.tecnico_assegnazione_pending_id = None
    if is_transfer:
        db_ddt.assegnazione_stato = "assegnato"
    else:
        db_ddt.assegnazione_stato = "da_assegnare"
        db_ddt.stato = "in_magazzino"
        db_ddt.stato_updated_at = datetime.now()

    db_ddt.assegnazioni_log = (db_ddt.assegnazioni_log or []) + [{
        "azione": "trasferimento_rifiutato" if is_transfer else "assegnazione_rifiutata",
        "by_user_id": current_user.id,
        "by_user_nome": current_user.nome_completo,
        "timestamp": datetime.now().isoformat()
    }]
...
@app.post("/ddt/{ddt_id}/request-transfer", response_model=schemas.RitiroProdottoResponse, tags=["DDT"])
def request_ddt_transfer(
    ddt_id: int,
    payload: schemas.DdtAssignRequest,
    request: Request,
    db: Session = Depends(database.get_db),
    ...
    """Richiede trasferimento DDT a un collega."""
    db_ddt = db.query(models.RitiroProdotto).filter(
        models.RitiroProdotto.id == ddt_id,
        models.RitiroProdotto.deleted_at.is_(None)
    ).first()
    ...
    if db_ddt.tecnico_assegnato_id != current_user.id and current_user.ruolo not in ["admin", "superadmin"]:
        raise HTTPException(status_code=403, detail="Non sei autorizzato a trasferire questo DDT")
    ...
    if tecnico.id == db_ddt.tecnico_assegnato_id:
        raise HTTPException(status_code=400, detail="Il DDT è già assegnato a questo tecnico")

    db_ddt.tecnico_assegnazione_pending_id = tecnico.id
    db_ddt.assegnazione_stato = "trasferimento_in_attesa"
    db_ddt.assegnazioni_log = (db_ddt.assegnazioni_log or []) + [{
        "azione": "trasferimento_richiesto",
        "da_tecnico_id": db_ddt.tecnico_assegnato_id,
        "a_tecnico_id": tecnico.id,
        "by_user_id": current_user.id,
        "by_user_nome": current_user.nome_completo,
    ...
    # Email a tecnico destinatario + responsabili DDT
    try:
        smtp_config = email_service.get_smtp_config(db)
        if smtp_config.get("username") and smtp_config.get("password"):
            ...
            if settings and settings.email_responsabile_ddt:
                recipients.update(parse_email_list(settings.email_responsabile_ddt))
            if recipients:
                subject = f"Richiesta trasferimento DDT {db_ddt.numero_ddt}"
                from_name = f"GIT - {settings.nome_azienda or 'GIT'} - DDT" if settings else "GIT - DDT"
                body_html = f"""
                <div style="font-family: Arial, sans-serif; color: #333;">
                    <p>È stata richiesta la presa in carico del DDT <strong>{db_ddt.numero_ddt}</strong>.</p>
                    <p>Cliente: <strong>{db_ddt.cliente_ragione_sociale}</strong></p>
                    <p>Richiedente: <strong>{current_user.nome_completo}</strong></p>
                </div>
                """
                for email_to in recipients:
                    email_service.send_email(
        ...
        logger.warning(f"Errore invio email trasferimento DDT: {e}")
