"""
Migrazione: Aggiunge il campo deleted_at alle tabelle clienti e magazzino per supportare soft delete
"""
...
def migrate():
    ...
    try:
        ...
        print("   - Se esegui da Docker, usa: docker-compose exec backend python /app/migrate_add_soft_delete.py")
        sys.exit(1)
    
    try:
        print("🔄 Inizio migrazione soft delete...")
        
        # 1. Aggiungi deleted_at a clienti se non esiste
        print("📋 Aggiunta campo deleted_at a clienti...")
        cursor.execute("""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name='clienti' AND column_name='deleted_at'
        """)
        if not cursor.fetchone():
            ...
            """)
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_clienti_deleted_at ON clienti(deleted_at);")
            print("✅ Colonna deleted_at aggiunta alla tabella clienti con indice")
        else:
            print("⏭️  Colonna deleted_at già presente in clienti")
        
        # 2. Aggiungi deleted_at a magazzino se non esiste
        print("📋 Aggiunta campo deleted_at a magazzino...")
        cursor.execute("""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name='magazzino' AND column_name='deleted_at'
        """)
        if not cursor.fetchone():
            cursor.execute("""
                ...
                ADD COLUMN deleted_at TIMESTAMP NULL;
            """)
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_magazzino_deleted_at ON magazzino(deleted_at);")
            print("✅ Colonna deleted_at aggiunta alla tabella magazzino con indice")
        else:
            print("⏭️  Colonna deleted_at già presente in magazzino")
        
        # 3. Aggiungi deleted_at a utenti se non esiste
        ...
        # 4. Aggiungi deleted_at a interventi se non esiste
